...
function loadTokenMap(): Map<string, string> {
  ...
  if (gheTokensRaw) {
    try {
      ...
      console.log(
        `[GHE Token Resolver] Loaded tokens for ${tokenMap.size} hostname(s): ${[...tokenMap.keys()].join(', ')}`,
      );
    } catch (e) {
      console.error(
        '[GHE Token Resolver] Failed to parse GHE_TOKENS as JSON. Expected format: {"hostname":"token",...}',
      );
    }
  }
 ...
 */
export function getTokenForHost(hostname: string): string | null {
 ...
 */
export function requireTokenForHost(hostname: string): string {
  const token = getTokenForHost(hostname);
  if (!token) {
    throw new Error(
      `No GHE token configured for hostname "${hostname}". ` +
