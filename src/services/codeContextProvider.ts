...
export interface CodeExample extends RepoKnowledge {
  ...
  similarity?: number; // optional for hybrid approach
}
...
export interface CodeContextOptions {
  ...
  org?: string;
  repo?: string;
  elementTypes?: string[]; // filter by 'class', 'function', 'test', etc.
  limit?: number;
  maxPerFile?: number; // max examples from same file
}
 ...
 */
export async function fetchDomainScopedCodeExamples(
...
function diversifyExamples(
  ...
  const domainCount = new Map<number, number>();
 ...
 */
export function formatCodeExamplesForPrompt(examples: CodeExample[]): string {
