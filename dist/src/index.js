...
const ontologyEngine_1 = require("./services/ontologyEngine");
    ...
    const server = http.createServer(async (req, res) => {
        ...
        `);
            ...
            if (url === '/api/domain-file-mappings' && method === 'GET') {
                ...
                const mappings = await client_1.pool.query(`
        ...
        `);
            ...
            if (url === '/api/domain-code-examples' && method === 'POST') {
                ...
                const body = await parseJsonBody(req);
                const examples = await (0, client_1.fetchDomainScopedCodeExamples)({
        ...
        `, [pr_url, channel_id, message_ts, JSON.stringify(review), JSON.stringify(reviewers)]);
          ...
          `, [limit]);
            ...
            if (url === '/api/resolve-rules' && method === 'POST') {
                ...
                const rules = await (0, ontologyEngine_1.resolveRulesForPR)(changed_files, diff_text || '');
                ...
                const unmatchedFiles = changed_files.filter((f) => !matchedFiles.has(f));
            ...
            if (url === '/api/ontology/domains' && method === 'GET') {
                ...
                const domains = await (0, ontologyEngine_1.getAllDomains)();
                res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            if (url === '/api/ontology/domains' && method === 'POST') {
                ...
                const domain = await (0, ontologyEngine_1.createDomain)(name, display_name, parent_id || null, description);
            ...
            if (url.startsWith('/api/ontology/rules') && method === 'GET') {
                ...
                const rules = await (0, ontologyEngine_1.listRules)({ domainId, teamOwner, enabledOnly: false });
                res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            if (url === '/api/ontology/rules' && method === 'POST') {
                ...
                const rule = await (0, ontologyEngine_1.createRule)(domain_id, rule_key, title, description, severity || 'high', team_owner);
                // Create associated matchers
                if (matchers && Array.isArray(matchers)) {
                    for (const m of matchers) {
                        await (0, ontologyEngine_1.createRuleMatcher)(rule.id, m.matcher_type, m.pattern, m.is_regex || false, m.priority || 0);
                    }
                }
                // Create associated domain file mappings
                if (file_mappings && Array.isArray(file_mappings)) {
                    for (const fm of file_mappings) {
                        await (0, ontologyEngine_1.createDomainFileMapping)(domain_id, fm.file_pattern, fm.priority || 0);
                    }
                }
            ...
            if (url.startsWith('/api/ontology/rules/') && method === 'PUT') {
                ...
                const body = await parseJsonBody(req);
                const updated = await (0, ontologyEngine_1.updateRule)(ruleId, body);
                if (!updated) {
                    res.writeHead(404, { 'Content-Type': 'application/json' });
            ...
            if (url.startsWith('/api/ontology/rules/') && method === 'DELETE') {
                ...
                const deleted = await (0, ontologyEngine_1.deleteRule)(ruleId);
