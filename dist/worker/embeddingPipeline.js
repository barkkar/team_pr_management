...
// Heroku API helpers
// ---------------------------------------------------------------------------
async function fetchUnembeddedReviews(limit = 50) {
    const response = await axios_1.default.get(`${HEROKU_API_URL}/api/unembedded-reviews`, {
        params: { limit },
...
async function fetchUnembeddedRepoKnowledge(limit = 50) {
    const response = await axios_1.default.get(`${HEROKU_API_URL}/api/unembedded-repo-knowledge`, {
        params: { limit },
        headers: { 'X-Worker-API-Key': WORKER_API_KEY },
        timeout: 30000,
    });
...
async function reportEmbeddings(embeddings) {
    await axios_1.default.post(`${HEROKU_API_URL}/api/embeddings`, { embeddings }, {
        headers: herokuHeaders(),
        timeout: 60000,
    });
}
async function reportRepoKnowledgeEmbeddings(updates) {
    await axios_1.default.post(`${HEROKU_API_URL}/api/repo-knowledge-embeddings`, { updates }, {
        headers: herokuHeaders(),
        timeout: 60000,
    });
}
...
async function embedRepoKnowledge() {
