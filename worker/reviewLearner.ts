...
function logError(msg: string, severity: 'warn' | 'error' | 'fatal' = 'error'): void {
  ...
  notifyError('ReviewLearner', msg, severity);
}
...
// ---------------------------------------------------------------------------
// LLM: Compare AI review vs peer comments and generate lessons
// ---------------------------------------------------------------------------
...
async function fetchPRsNeedingLessons(): Promise<any[]> {
...
async function reportLessons(prUrl: string, aiReview: any, peerComments: any[], lessons: Lessons): Promise<void> {
...
async function runWatchMode(): Promise<void> {
  ...
  await runWorker();
  setInterval(async () => {
    try { await runWorker(); } catch (e) { logError(`Run failed: ${e}`); }
  }, POLL_INTERVAL_MS);
}
